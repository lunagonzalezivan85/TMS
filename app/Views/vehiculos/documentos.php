        ...
        <div class="card-body p-3 p-lg-4 text-white">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <span class="badge fs-6 fw-bold px-3 py-2 rounded-pill" style="background: rgba(255,255,255,.2); backdrop-filter: blur(10px); letter-spacing: 2px;">
                        <i class="fas fa-id-card me-1"></i><?= esc($vehiculo['placa']) ?>
                    </span>
                    <span class="fw-bold fs-5"><?= esc($vehiculo['marca']) ?> <?= esc($vehiculo['modelo']) ?> <span class="opacity-75 fw-normal small">(<?= esc($vehiculo['anio']) ?>)</span></span>
                    <span class="badge bg-<?= $badgeClass ?>"><?= esc($vehiculo['estado']) ?></span>
                </div>
                <div class="d-flex gap-2 align-items-center">
                    <button type="button" class="btn btn-light btn-sm fw-semibold px-3" data-bs-toggle="modal" data-bs-target="#modalSubirDocumento">
                        <i class="fas fa-upload me-1"></i>Subir
                    </button>
                    <a href="<?= base_url('vehiculos/show/' . $vehiculo['id']) ?>" class="btn btn-outline-light btn-sm px-3">
                        <i class="fas fa-arrow-left me-1"></i>Volver
                    </a>
                </div>
            </div>
            <div class="d-flex gap-3 mt-2 flex-wrap small opacity-90">
                <span><i class="fas fa-road me-1"></i><?= number_format($vehiculo['kilometraje'] ?? 0) ?> km</span>
                <span><i class="fas fa-tag me-1"></i><?= esc($vehiculo['codigo_consecutivo'] ?? '—') ?></span>
            </div>
        </div>
    </div>
