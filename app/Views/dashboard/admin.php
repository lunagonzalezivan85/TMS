...
<style>
    ...
    .cmd-item:hover, .cmd-item.active { background: #f0fdf9; }
    .cmd-item i { width: 24px; text-align: center; color: #059669; }
    .kbd-hint {
        display: inline-flex;
        align-items: center;
        gap: 0.25rem;
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex flex-wrap gap-2 mb-4">
        <a href="<?= base_url('vehiculos') ?>" class="chip text-decoration-none text-dark">
            ...
            <span>Vehículos</span>
            <span class="chip-count bg-success text-white"><?= $stats['total_vehicles'] ?></span>
        </a>
        ...
        <a href="<?= base_url('conductores') ?>" class="chip text-decoration-none text-dark">
            <i class="fas fa-users text-info"></i>
            <span>Conductores</span>
            <span class="chip-count bg-info text-white"><?= $stats['total_conductores'] ?></span>
        </a>
        <a href="<?= base_url('combustible') ?>" class="chip text-decoration-none text-dark">
            <i class="fas fa-gas-pump text-danger"></i>
            <span>Combustible</span>
            <span class="chip-count bg-danger text-white"><?= number_format($stats['total_galones'], 1) ?> gal</span>
        </a>
        <a href="<?= base_url('solicitudes') ?>" class="chip text-decoration-none text-dark">
...
function renderItems(filter = '') {
    ...
    list.innerHTML = '';
    const filtered = comandos.filter(c => c.label.toLowerCase().includes(filter.toLowerCase()));
